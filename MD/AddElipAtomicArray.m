<<<<<<< HEAD
function AddElipAtomicArray(rad_x, rad_y, X0, Y0, VX0, VY0, InitDist, Temp, Type)
global C
...
L = (2*rad_x - 1) * AtomSpacing;
W = (2*rad_y - 1) * AtomSpacing;

xp(1, :) = linspace(-L/2, L/2, 2*rad_x);
yp(1, :) = linspace(-W/2, W/2, 2*rad_y);

numAtoms = 0;
for i = 1:2*rad_x
    for j = 1:2*rad_y
        if (xp(i)/(rad_x*AtomSpacing))^2 + (yp(j)/(rad_y*AtomSpacing))^2 <= 1
            numAtoms = numAtoms+1;
